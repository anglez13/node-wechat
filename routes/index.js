...
module.exports = function(app) {
  ...
  app.post('/upload', function (req, res) {
    for (var i in req.files) {
      if (req.files[i].size == 0){
        // 使用同步方式删除一个文件
        fs.unlinkSync(req.files[i].path);                    //删除空文件
  ...
  app.get('/usersm', checkLoginAdmin)
  ...
  app.get('/edit/:name', checkLoginAdmin);
  app.get('/edit/:name', function (req, res) {
    //var currentUser = req.session.user;
    User.edit(req.params.name, function (err,user) {
      if (err) {
        req.flash('error', err); 
        return res.redirect('back');
      }
  ...
  app.post('/edit/:name', function (req, res) {
    ...
    User.update(req.params.name, req.body.email, function (err) {
      ...
      req.flash('success', '修改成功!');
      res.redirect('/usersm');//成功！返回文章页
    });
  });
  
  
  app.get('/remove/:name', checkLoginAdmin);
