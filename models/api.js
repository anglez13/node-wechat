...
Weixin.prototype.subEventMsg = function (callback) {
	...
	emitter.on('weixinSubEventMsg', callback);
...
Weixin.prototype.unsubEventMsg = function (callback) {
	...
	emitter.on('weixinUnSubEventMsg', callback);
...
Weixin.prototype.scanEventMsg = function (callback) {
	...
	emitter.on('weixinScanEventMsg', callback);
...
Weixin.prototype.enterEventMsg = function (callback) {
	...
	emitter.on('weixnEnterEventMsg', callback);
...
Weixin.prototype.locationEventMsg = function (callback) {
	...
	emitter.on('weixinLocationEventMsg', callback);
...
Weixin.prototype.clickEventMsg = function (callback) {
	...
	emitter.on('weixinClickEventMsg', callback);
...
Weixin.prototype.parseEventMsg = function() {
	var event = this.data.Event[0];
	console.log(event);
	switch (event) {
        case 'subscribe':
            this.parseSubEventMsg();
            break;
        case 'unsubscribe':
            this.parseUnsubEventMsg();
            break;
        case 'scan':
            this.parseScanEventMsg();
            break;
        case 'ENTER':
            this.parseEnterEventMsg();
            break;
        case 'LOCATION':
            this.parseLocationEventMsg();
            break;
        case 'CLICK':
            this.parseClickEventMsg();
            break;
    }
...
//处理订阅事件消息
Weixin.prototype.parseSubEventMsg = function () {
    // 扫码，未关注，先关注后发场景值
    ...
    emitter.emit('weixinSubEventMsg', msg);
    
	return this;
}

//处理取消订阅事件消息
 Weixin.prototype.parseUnsubEventMsg = function () {
    console.log(this.data);
	var eventKey = '';
	if (this.data.EventKey) {
		eventKey = this.data.EventKey[0];
	...
	emitter.emit('weixinUnSubEventMsg', msg);
    
	return this;
}

//处理扫码事件消息
Weixin.prototype.parseScanEventMsg = function () {
    
	emitter.emit('weixinScanEventMsg', this.msg);
    
	return this;
}

//处理进入会话事件消息
Weixin.prototype.parseEnterEventMsg = function () {
    
	emitter.emit('weixinEnterEventMsg', this.msg);
    
	return this;
}

//处理上报地理位置消息
Weixin.prototype.parseLocationEventMsg = function () {
    
	emitter.emit('weixinLocationEventMsg', this.msg);
    
	return this;
}

//处理自定义菜单事件
Weixin.prototype.parseClickEventMsg = function () {
    console.log(this.data);
	var eventKey = '';
	...
	emitter.emit('weixinClickEventMsg', msg);
